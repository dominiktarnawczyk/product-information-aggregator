...
| `AVAILABILITY_LATENCY_MIN`         | Min latency in milliseconds | `100`   | `100`       |
| `AVAILABILITY_LATENCY_MAX`         | Max latency in milliseconds | `100`   | `500`       |
...
| Environment Variable           | Description                 | Default | Example     |
|--------------------------------|-----------------------------|---------|-------------|
| `PRICING_INTERCEPTOR_ENABLED`  | Enable/disable interceptor  | `true`  | `false`     |
| `PRICING_LATENCY_MIN`          | Min latency in milliseconds | `80`    | `100`       |
| `PRICING_LATENCY_MAX`          | Max latency in milliseconds | `80`    | `500`       |
| `PRICING_FAILURE_RATE`         | Failure rate (0.0-1.0)      | `0.005` | `0.2` (20%) |
| `PRICING_ERROR_STATUS_CODE`    | HTTP status for failures    | `500`   | `503`       |
...
### Test with Latency (200-500ms)
```bash
PRICING_INTERCEPTOR_ENABLED=true \
PRICING_LATENCY_MIN=200 \
PRICING_LATENCY_MAX=500 \
./gradlew :mocks:bootRun
```
...
```bash
# Should take 200-500ms to respond (without customer-id)
time curl "http://localhost:8081/api/pricing?product-id=TEST-123&market-code=en-GB"

# Should take 200-500ms to respond (with customer-id for discount)
time curl "http://localhost:8081/api/pricing?product-id=TEST-123&market-code=en-GB&customer-id=CUSTOMER-456"
```
...
for i in {1..10}; do
  echo "Request $i:"
  curl -w "\nHTTP Status: %{http_code}\n" "http://localhost:8081/api/pricing?product-id=TEST-123&market-code=en-GB&customer-id=CUSTOMER-456"
  echo "---"
done
```
